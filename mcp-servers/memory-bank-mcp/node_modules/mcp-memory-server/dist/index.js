...
function createDefaultMetadata(sourceFile) {
    return {
        sourceFile,
        confidence: 'high',
        accessCount: 0,
...
function createEntity(name, entityType, observations, metadata) {
...
class KnowledgeGraphManager {
    ...
    async acquireWriteLock() {
        while (this.writeLock) {
            await new Promise(resolve => this.writeLockQueue.push(resolve));
        }
        this.writeLock = true;
    }
    releaseWriteLock() {
    ...
    async loadGraph() {
        try {
            ...
            return lines.reduce((graph, line, index) => {
                try {
    ...
    async createRelations(relations) {
        ...
        try {
            ...
            for (const relation of relations) {
                ...
                const exists = graph.relations.some(r => r.from === relation.from &&
                    r.to === relation.to &&
    ...
    async getStaleEntities(days, entityType) {
        const graph = await this.loadGraph();
        const now = new Date();
    ...
    async validateComponentProps(componentName, propsToCheck) {
        const graph = await this.loadGraph();
    ...
    async getFrequentlyUsed(minAccessCount, entityType) {
        const graph = await this.loadGraph();
    ...
    async incrementAccessCount(entityName) {
        await this.acquireWriteLock();
        try {
            const graph = await this.loadGraph();
    ...
    async verifyGraphIntegrity(maxSuggestions = 3) {
        const graph = await this.loadGraph();
        const entityNames = new Set(graph.entities.map(e => e.name));
        const orphanedRelations = [];
        const uniqueOrphanedEntities = new Set();
    ...
    findSimilarEntities(targetName, entities, maxSuggestions) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        case "create_entities":
            ...
            const fullEntities = args.entities.map(e => createEntity(e.name, e.entityType, e.observations));
            return { content: [{ type: "text", text: JSON.stringify(await knowledgeGraphManager.createEntities(fullEntities), null, 2) }] };
        case "create_relations":
            const relResult = await knowledgeGraphManager.createRelations(args.relations);
            let relMessage = "";
        ...
        case "add_observations":
            const obsResult = await knowledgeGraphManager.addObservations(args.observations);
            let obsMessage = "";
        ...
        case "get_stale_entities":
            return { content: [{ type: "text", text: JSON.stringify(await knowledgeGraphManager.getStaleEntities(args.days, args.entityType), null, 2) }] };
        case "validate_component_props":
            const validationResult = await knowledgeGraphManager.validateComponentProps(args.componentName, args.propsToCheck);
            if (!validationResult) {
        ...
        case "update_entity":
            const updatedEntity = await knowledgeGraphManager.updateEntity(args.name, {
        ...
        case "get_frequently_used":
            return { content: [{ type: "text", text: JSON.stringify(await knowledgeGraphManager.getFrequentlyUsed(args.minAccessCount, args.entityType), null, 2) }] };
        case "verify_graph_integrity":
            const integrityResult = await knowledgeGraphManager.verifyGraphIntegrity(args.maxSuggestions);
