...
| **postgres** | Database | Secure database interactions | Query execution, schema inspection, read-only safety |
...
| **docker-hub-remote** | Official | Docker Hub integration | Search and retrieve container images from Docker Hub |
...
This repository is also configured to work with the Gemini CLI. To use the MCP servers with Gemini, you will need to use the `--mcp-config` flag to specify the `.gemini.json` configuration file.

```bash
gemini --mcp-config .gemini.json
```

This will start the Gemini CLI with all the MCP servers configured and ready to use.

### Gemini CLI setup

The `.gemini.json` file in this repository is pre-configured to work with the Gemini CLI. To use it, you can either:

1.  **Set it as your default configuration:**
    ```bash
    cp .gemini.json ~/.gemini/settings.json
    ```
2.  **Use it for a specific project:**
    *   Place the `.gemini.json` file in the root of your project.
    *   The Gemini CLI will automatically detect and use it.

This file configures all the MCP servers with the correct paths and settings, ensuring they work seamlessly with the Gemini CLI.
